...
/**
 * @author cxm "可见我的"页面列表适配器 2015-03-09 20:52:41
 */
public class CanSeeMeListAdapter extends BaseAdapter {
	...
	public CanSeeMeListAdapter(Context context, List<UserBean> list) {
		super();
		this.context = context;
		CanSeeMeListAdapter.userBeans = list;
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = LayoutInflater.from(context).inflate(
					R.layout.listitem_canseeme, new RelativeLayout(context));
			// 单个listview各属性添加值
			...
			holder.chooseBox = (CheckBox) convertView
					.findViewById(R.id.choose_box);
			holders.add(holder);
			convertView.setTag(holder);
		} else {
			holder = (ViewHolder) convertView.getTag();
		}
		holder.chooseBox.setChecked(data.isCanSeeMe());
		holder.chooseBox.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				data.setCanSeeMe(((CheckBox) v).isChecked());// 更改数据源的可见状态
			}
		});
		// 给控件设置内容
